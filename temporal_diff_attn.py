...
def _sanity_check() -> None:
    ...
    attn = TemporalDiffAttn(use_temporal_bias=True)
    y = attn(x)
    ...
    print(f"[OK] with temporal bias: sigma_signal={attn.sigma_signal.item():.2f}, "
          f"sigma_noise={attn.sigma_noise.item():.2f}, lambda={attn.last_lambda.item():.4f}")
